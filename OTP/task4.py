...
def main(): 
    ...
    with open(in_file_path1, mode='rb') as plaintext2:
    ...
    key = get_random_bytes(len(image1))
    ...
    write_file(encrypted_file2, header1, ciphertext2)
    ...
    decrypted_file = in_file_path1.split(".")[0] + "_two_time_pad_decrypted.bmp"
    write_file(decrypted_file, header1, decrypted_ciphertext)

    # Write the decrypted image to a bmp file
    decrypted_file = in_file_path2.split(".")[0] + "_two_time_pad_decrypted.bmp"
    write_file(decrypted_file, header2, decrypted_ciphertext)
